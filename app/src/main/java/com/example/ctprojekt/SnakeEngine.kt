...
class SnakeEngine(context: Context) : SurfaceView(context), Runnable{
    ...
    private var eat_bob = -1
    private var snake_crash = -1
    ...
    fun update() {
        ...
        if (detectDeath()) {
            //start again
            soundPool!!.play(snake_crash, 1f, 1f, 0, 0, 1f)
            newGame()
        }
    }
    fun draw(){
        if (surfaceHolder!!.surface.isValid) {
            ...
            for (i in 0 until snakeLength) {
                canvas!!.drawRect((snakeXs[i] * blockSize).toFloat(),
            ...
            // Draw Bob
            canvas!!.drawRect((bobX * blockSize).toFloat(),
